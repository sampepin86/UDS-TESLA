...
class PCANBasic:
    ...
    def __init__(self):
        ...
        if hasattr(sys, '_MEIPASS'):
            # Patch for running in a temp directory as packaged by PyInstaller
            tempDir = sys._MEIPASS
            lib_path = os.path.join(tempDir, "PCANBasic.dll")
            self.__m_dllBasic = windll.LoadLibrary(lib_path)
        else:
            # For loading from the local directory
            if sys.platform.startswith('win'):
                dll_file = os.path.split(__file__)[0] + "\PCANBasic"
                self.__m_dllBasic = windll.LoadLibrary(dll_file)
    ...
    def GetValue(
        ...
        """
        try:
#            if Parameter == PCAN_API_VERSION or Parameter == PCAN_HARDWARE_NAME or Parameter == PCAN_CHANNEL_VERSION or Parameter == PCAN_LOG_LOCATION or Parameter == PCAN_TRACE_LOCATION or Parameter == PCAN_SPEED_QUERY_FD:
